...
class PayTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->payment->setTransport($this->transport);
        parent::setUp();
    }
    ...
    {
        $token=uniqid();
        $this->transport->body=json_encode(['token'=>$token,'status'=>1]);
        $request=$this->payment->request('https://dpe.ac/callback',100000);
        $this->assertEquals($request['token'],$token);
        $this->assertContains($token,$this->payment->redirectUrl());
    }

    /** @test */
    public function it_could_verify_success_transaction()
    {
        $_GET['invoice_id']=uniqid();
        $_GET['status']=1;
        $_GET['token']=uniqid();
        $amount = 10000;
        ...
        $result = $this->payment->verify($amount,$_GET['invoice_id']);
        self::assertEquals($exceptedData['cardNumber'],$result['card_number']);
        self::assertEquals($exceptedData['transId'],$result['transaction_id']);
    }
}
